...
/*
  PRELOADER — real vector paths extracted from logo SVG clipPaths
  ───────────────────────────────────────────────────────────────
  Bird  → from <clipPath id="clip-path">   viewBox ~0 0 175 92
  Text  → from <clipPath id="clip-path-2"> viewBox ~135 285 235 75

  Animation sequence
  ──────────────────
  0.3s   bird stroke starts drawing   (pink → white)
  0.65s  wordmark stroke starts drawing
  2.0s   filled logo.svg fades in, strokes fade out
  2.4s   tagline fades in
  3.8s   whole screen fades out → onComplete()
*/
...
export default function Preloader({ onComplete }) {
  ...
  useEffect(() => {
    ...
    const birdPaths = birdRef.current?.querySelectorAll("path");
    const wordPaths = wordRef.current?.querySelectorAll("path");
    if (!birdPaths || !wordPaths) return;
    ...
    setupPaths(birdPaths);
    setupPaths(wordPaths);

    const tl = gsap.timeline();

    // 1. Draw bird outline
    tl.to(Array.from(birdPaths), {
      strokeDashoffset: 0,
      duration: 1.6,
      stagger: 0.04,
      ease: "power2.inOut",
    ...
    // 2. Draw wordmark outline (slightly delayed)
    tl.to(Array.from(wordPaths), {
      strokeDashoffset: 0,
      duration: 1.4,
      stagger: 0.02,
      ease: "power2.inOut",
    ...
    // 3. Shift stroke color pink → white
    tl.to([birdRef.current, wordRef.current], {
      "--stroke-color": "#ffffff",
      duration: 0.7,
      ease: "power2.out",
    }, 1.3);

    // 4. Fade in filled logo, fade out strokes
    tl.to(filledRef.current, {
      opacity: 1,
      duration: 0.5,
      ease: "power2.out",
    }, 2.0);
  ...
  return (
    <div ref={wrapRef} className={styles.wrap}>
      ...
      {/* Bird stroke — viewBox frames the 0 0 175 92 region of the logo */}
      <svg
        ref={birdRef}
        className={styles.birdOutline}
        viewBox="-2 -2 180 96"
        fill="none"
      ...
      {/* Wordmark stroke — viewBox frames x≈135 y≈285 w≈235 h≈75 */}
      <svg
        ref={wordRef}
        className={styles.wordOutline}
      ...
      <div ref={filledRef} className={styles.filled}>
        <img
          src="/images/pickar.png"
          alt="Pickar"
